...
class Admin(tk.Frame):
    def __init__(self,parent,controller):
        ...
        border=LabelFrame(self,text="Login",font=('cooper black', 30, 'bold'), bg='black',fg='white', relief=RAISED)
...
class studentsinfodb(tk.Frame):
    def __init__(self,parent,controller):
        tk.Frame.__init__(self,parent)
        
        self.title('student enrollment')
        self.geometry('1300x650')
        label1 = Label(self, text='Students Course Enrollment System', font=('cooper black', 30, 'bold'), bg='black',
                       fg='white', relief=RAISED)
        label1.place(x=230,y=230)

        button=Button(self,text="Home",font=('cooper black', 30, 'bold'), bg='black',fg='white', relief=RAISED,command=lambda:controller.show_frame(Admin))
        button.place(x=650,y=450)
        
        formFrame = Frame(self, bg='cyan')
        formFrame.place(x=10, y=80, width=400, height=550)

        dataFrame = Frame(self, bg='cyan')
        ...
        label3 = Label(dataFrame, text='Students Information Display', bg='black', fg='white',
                       font=('elephant', 15, 'bold'), bd=5, relief=RAISED)
        label3.grid(row=0, columnspan=5, padx=250, pady=15)

        tableFrame = Frame(dataFrame, bg='cyan', bd=5, relief=GROOVE)
        ...
        # working with frame
        label2 = Label(formFrame, text='Students Enrollment', bg='black', fg='white',
                       font=('elephant', 15, 'bold'), bd=5, relief=RAISED)
        label2.grid(row=0, columnspan=5, padx=50, pady=15)

        # roll number
        lbl_roolno = Label(formFrame, text='Roll Number', font=('HP Simplified Japan', 15, 'bold'), bg='cyan',
                           fg='black')
        lbl_roolno.grid(row=1, column=0, sticky='W')

        entry_roolno = Entry(formFrame, textvariable=self.rool_no_var, font=('HP Simplified Japan', 15, 'bold'))
        entry_roolno.grid(row=1, column=1, pady=10)

        # first name
        lbl_fname = Label(formFrame, text='First Name', font=('HP Simplified Japan', 15, 'bold'), bg='cyan',
                          fg='black')
        ...
        # last name
        lbl_lname = Label(formFrame, text='Last name', font=('HP Simplified Japan', 15, 'bold'), bg='cyan', fg='black')
        ...
        # emaild
        lbl_email = Label(formFrame, text='Email ID', font=('HP Simplified Japan', 15, 'bold'), bg='cyan', fg='black')
        ...
        # contact no
        lbl_cno = Label(formFrame, text='Contact No', font=('HP Simplified Japan', 15, 'bold'), bg='cyan', fg='black')
        ...
        # course name
        lbl_cname = Label(formFrame, text='Course Name', font=('HP Simplified Japan', 15, 'bold'), bg='cyan',
                          fg='black')
        ...
        # location
        lbl_loc = Label(formFrame, text='location', font=('HP Simplified Japan', 15, 'bold'), bg='cyan', fg='black')
        ...
        btnFrame = Frame(formFrame, bg='cyan', bd=5, relief='groove')
        btnFrame.place(x=10, y=380, height=100, width=370)

        # Add Button
        btnAdd = Button(btnFrame, text='Add', command=self.adding_data, font=('HP Simplified Japan', 15, 'bold'),
                        bg='black', fg='white')
        btnAdd.grid(row=0, column=0, padx=10, pady=20)

        # Update Button
        btnUpdate = Button(btnFrame, text='Update', command=self.update_data, font=('HP Simplified Japan', 15, 'bold'),
                           bg='black', fg='white')
        btnUpdate.grid(row=0, column=1, padx=10, pady=20)

        # Delete Button
        btnDelete = Button(btnFrame, text='Delete', command=self.delete_data, font=('HP Simplified Japan', 15, 'bold'),
                           bg='black', fg='white')
        btnDelete.grid(row=0, column=2, padx=10, pady=20)

        # clear
        btnClear = Button(btnFrame, command=self.clearing_data, text='Clear', font=('HP Simplified Japan', 15, 'bold'),bg='black', fg='white')
...
class Application(tk.Tk):
    def __init__(self,*args,**kwargs):
        ...
        window.grid_rowconfigure(0,minimize=500)
        window.grid_columnconfigure(0,minimize=800)

        self.frames={}
        for F in (Admin,studentsinfodb):
            frame=F(window,self)
            self.frame[F]=frame
